...
class PredictionPipeline:
    ...
    def load_model_and_preprocessor(self):
        try:
            ...
            preprocessor = self.utils.load_object(self.config.preprocessor_path)
    ...
    def get_prediction_dataframe(self, input_file_path: str):
        try:
            ...
            df.to_csv(self.config.prediction_file_path, index=False)

            return self.config.prediction_file_path

        except Exception as e:
            raise CustomException(e, sys)

    def run_pipeline(self, input_file_path: str):
        try:
            output_path = self.get_prediction_dataframe(input_file_path)
            return output_path
        except Exception as e:
            raise CustomException(e, sys)
